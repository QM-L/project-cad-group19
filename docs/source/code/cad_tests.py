...
def linear_regression():
    ...
    # Linear regression
    matrix = np.dot(trainXones.T,trainXones)
    column = np.dot(trainXones.T,trainY)

    Theta,_ = reg.ls_solve(matrix, column)
    ...
    PredictedY_validation = validationX*Theta[0]+Theta[1]
    PredictedY_test = testX*Theta[0]+Theta[1]

    E_validation = np.mean(np.square(validationY - PredictedY_validation))
    E_test = np.mean(np.square(testY - PredictedY_test))
...
def quadratic_regression():
    ...
    trainX = train_data[:,0].reshape(-1,1)
    trainX_squared = np.square(trainX)
    trainX_stack = np.hstack((trainX,trainX_squared)) 
    trainX_full = util.addones(trainX_stack)
    trainY = train_data[:,1].reshape(-1,1)

    # Linear regression
    matrix = np.dot(trainX_full.T,trainX_full)
    column = np.dot(trainX_full.T,trainY)
    ...
    fig1 = plt.figure(figsize=(10,10))
    ax1 = fig1.add_subplot(111)
    util.plot_regression(trainX_stack, trainY, Theta, ax1)
    ax1.grid()
    ax1.set_xlabel('x')
    ax1.set_ylabel('y')
    ax1.legend(('Original data', 'Regression curve', 'Predicted Data', 'Error'))
    ...
    testX = test_data[:,0].reshape(-1,1)
    testX_squared = np.square(testX)
    testX_stack = np.hstack((testX,testX_squared))
    testY = test_data[:,1].reshape(-1,1)

    fig2 = plt.figure(figsize=(10,10))
    ax2 = fig2.add_subplot(111)
    util.plot_regression(testX_stack, testY, Theta, ax2)
    ax2.grid()
    ax2.set_xlabel('x')
    ax2.set_ylabel('y')
    ax2.legend(('Original data', 'Regression curve', 'Predicted Data', 'Error'))
    ...
    PredictedY_validation = validationX*Theta[0]+np.square(validationX)*Theta[1]+Theta[2]
    PredictedY_test = testX*Theta[0]+np.square(testX)*Theta[1]+Theta[2]
...
def covariance_matrix_test():
    ...
    print("sigma valid") if np.allclose(sigma_hat, sigma_hat.T) == True else print("sigma not Transpose symmetric.")

    print("X:",X)
    print("Y:",Y)
    return X, Y, sigma_hat
...
def eigen_vecval_test(sigma):
    ...
    # Property 1: Check orthogonality of eigenvectors
    orthogonality = np.dot(v[:, 0], v[:, 1])  # Should be close to zero if orthogonal
    print(f"Dot product of eigenvectors (should be close to 0 if orthogonal): {orthogonality}")
    
    # Property 2: Check normalization of eigenvectors
    norms = np.linalg.norm(v, axis=0)
    print(f"Norms of eigenvectors (should be 1 if normalized): {norms}")
    
    # Largest and smallest eigenvalue
    largest_eigenvalue = w[0]
    smallest_eigenvalue = w[-1]
